...
public class PKFxMenus : MonoBehaviour {
	...
	public static void MoveSounds()
	{
		...
		foreach (string file in files)
		{
			...
			string dirName = System.IO.Path.GetDirectoryName(Application.dataPath + "/Resources/PKFxSounds/" + pathName);
	...
	public static void CreateShadersAssets()
	{
		...
		{
			string pathName = file.Substring((Application.streamingAssetsPath + "/PackFx/").Length);
			string dirName = System.IO.Path.GetDirectoryName(Application.dataPath + "/Resources/PKFxShaders/" + pathName);
			if (!Directory.Exists(dirName))
				Directory.CreateDirectory(dirName);
			string filePath = dirName + "/" + System.IO.Path.GetFileNameWithoutExtension(pathName) + ".asset";
