...
#region Global conf
	...
	public class PKFxConf
	{
		...
		public void Save()
		{
			...
			if (!Directory.Exists(System.IO.Path.GetDirectoryName(confPath)))
			    Directory.CreateDirectory(System.IO.Path.GetDirectoryName(confPath));
...
#region dll types
	...
	public static string				m_LogFilePath = System.IO.Path.GetFullPath(System.IO.Path.Combine(Application.dataPath, "../popcorn.htm"));
	...
	{
		WWW www = new WWW(System.IO.Path.Combine(Application.streamingAssetsPath, "PKconfig.cfg"));
		while (!www.isDone)
			yield return www;
